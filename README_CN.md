...
* **仅显示选中行**——没有 `LINES` 的块（整文件嵌入），拖选多个行号后在浮层点「仅显示选中行」：块被改写为 `LINES: "<选区>"`（若存在 `HIDE` 键则一并移除）。`Ctrl+Z` 可撤销。也可用命令**仅显示选中的代码行**。已有 `LINES` 的块不出现该按钮，绝不静默改写既有 `LINES`。
* **HIDE 转 LINES**——命令**将 HIDE 转为 LINES**（也在编辑器右键菜单中）把 `HIDE` 折叠回等价的 `LINES` 值（渲染逐行一致，`HIDE` 键移除；无 `HIDE` 的块为空操作）。
* **临时展开隐藏段**——每个折叠的 `...` 段在行号区显示 `▸` 符号（行号关闭时点击 `...` 行本身）。点击后在原位临时展开隐藏行（半透明「幽灵行」，纯视图态、绝不写文件；任何重渲染都会自动收起）。展开区间以 `▾`（首行）和 `▴`（末行）括号标记，点击任一端收起。
* **展开态恢复隐藏行**——在展开段内：单击幽灵行行号立即恢复该行，或拖选多行后点「恢复选中行」确认（`Ctrl`/`Cmd`+拖选跳过确认）。恢复以**单次最小 diff** 写回 `LINES ∪ 选中` 且 `HIDE − 选中`（无 `LINES` 的块绝不创建 `LINES` 键）。
